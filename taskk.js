...
*/
function sostavChisla(arr, x) {

  const combinations = [];
  // [1, 2, 3, 4, 5, 6, 7, 8] , 8
  function generateCombinations(combination) {

    const currentCombinationSum = combination.reduce((a, b) => a + b, 0);

    if (currentCombinationSum === x) {
      if (!combinations.some(function (comb) {
        return comb.length === combination.length
          && comb.sort().every((value, index) => value === combination.sort()[index])
      })) {
